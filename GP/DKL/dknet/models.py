...
class CoreNN:
	...
	def first_run(self,X,Y,load_path=None):
		A=X
		if not self.layers:
			brflag=True
		else:
		...
		while not brflag:
			...
			if self.layers[i].trainable:
				if load_path is None or i==len(self.layers)-1:	
					self.layers[i].initialize_ws()
				else:
					self.layers[i].load_ws(load_path, j)	
	...
	def save_weights(self,save_path):
		...
		while not brflag:
			if self.layers[i].trainable:
				self.layers[i].save_ws(save_path, j)
				j += 1	
			i+=1
...
class NNRegressor(CoreNN):
	def __init__(self,layers=[64],opt=None,maxiter=200,batch_size=64,gp=True,verbose=False):
		CoreNN.__init__(self,layers,mse_loss)
		if gp:
