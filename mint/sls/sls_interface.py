...
class SLSInterface(MachineInterface):
    name = 'SLSInterface'

    def __init__(self, args=None):
        super(SLSInterface, self).__init__(args=args)
        self.config_dir = os.path.join(self.config_dir, "SLS")
    ...
    def get_charge(self):
        return self.get_value("ARS07-DPCT-0000:CURR")
    ...
    def logbook(self, gui):
        ...
        cmd =     cmd + '-a "Titel='         + titel         + '" '
        cmd =     cmd + '-a "Eintrag='       + eintrag       + '" '
        cmd =     cmd + '-a "Autor='         + autor         + '" '
    ...
    def get_preset_settings(self):
        presets = {
                "Test": [
                        {"display": "Playground Gauss", "filename": "test.json"}
                ]
                # "Aramis 2": [
