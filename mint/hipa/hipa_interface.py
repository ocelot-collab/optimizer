...
class HipaInterface(MachineInterface):
    name = 'HipaInterface'

    def __init__(self, args=None):
        super(HipaInterface, self).__init__(args=args)
        self.config_dir = os.path.join(self.config_dir, "hipa")
    ...
    def get_charge(self):
        return self.get_value("MXC1:IST:2")
    ...
    def logbook(self, gui):
        ...
        cmd =     cmd + '-a "Titel='         + title         + '" '
        cmd =     cmd + '-a "Eintrag='      + category      + '" '
        cmd =     cmd + '-a "Autor='   + application   + '" '
    ...
    def get_preset_settings(self):
        presets = {
                "Test": [
                        {"display": "Playground", "filename": "Playground.json"}
