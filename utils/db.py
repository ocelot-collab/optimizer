...
class PerfDB:
    ...
    def new_action(self, tuning_id, start_sase, end_sase):
        ...
        with self.db:
            cursor = self.db.cursor()
            cursor.execute("insert into ACTIONS(TUNING_ID,SASE_START,SASE_END) VALUES(?,?,?)",
                           (tuning_id, start_sase, end_sase))
    ...
    def add_action_parameters(self, tuning_id, action_id, param_names, start_vals, end_vals):
        ...
        with self.db:
            ...
            for i in range(len(param_names)):
                cursor.execute(
                    "insert into PARAMETERS(TUNING_ID,ACTION_ID, PAR_NAME,PAR_START_VALUE,PAR_END_VALUE) VALUES(?,?,?,?,?)",
                    (tuning_id, action_id, param_names[i], start_vals[i], end_vals[i]))

    def get_action_parameters(self, tuning_id, action_id):
        cursor = self.db.cursor()
        cursor.execute("select * from PARAMETERS WHERE TUNING_ID=:tid and ACTION_ID = :aid",
                       {'tid': tuning_id, 'aid': action_id})
...
def test_new_tunings(dbname):
    ...
    db.new_tuning(
        {'wl': 13.6, 'charge': 0.1, 'comment': 'test tuning'})  # creates new tuning record (e.g. for each shift);
...
def test_add_action_parameters(dbname):
    ...
    db.add_action_parameters(tune_id, action_id, param_names=["H1", "Q1", "test"], start_vals=[0.1, 0.2, "test"],
                             end_vals=[1.1, 1.3, "gh"])
